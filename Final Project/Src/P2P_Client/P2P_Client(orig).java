	...
	*/
		...
		while(true) {
			if (response.contains("200")) {
				...
				try{
				// receive file
					filename = in.readLine();   //NEED TO CHANGE (?)
				int current = 0;
				  byte [] mybytearray  = new byte [6022386];
				  InputStream inn = P2PSocket.getInputStream();
				  fos = new FileOutputStream("./sharedDir/"+contentName);
				  bos = new BufferedOutputStream(fos);
				  bytesRead = inn.read(mybytearray,0,mybytearray.length);
				  current = bytesRead;
				
				do {
					bytesRead = inn.read(mybytearray, current, (mybytearray.length-current));
				...
				File imageFile = new File(sharedDir+"/"+filename); 
